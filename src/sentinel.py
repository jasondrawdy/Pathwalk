...
class Sentinel(object):
    ...
    def __init__(self: "Sentinel", id: str = generators.generate_id(10), verbose: bool = False):
        ...
        self.authorized = False # Flag which tells the sentinel if it is allowed to scan the system.
        self.monitoring = False # Flag which tells the sentinel if it is currently monitoring the system.
        self.log = Logger(__name__, write_output=False) # Logger for passing information to the console and etc.
        self._garbage_paths = [ "__pycache__", ".DS_Store"]
    ...
    def _find_garbage(self: "Sentinel", path: str) -> int:
        ...
        for root, dirs, files in os.walk(path):
            for file in files:
                if file in self._garbage_paths:
                    try:
                        os.remove(os.path.join(root, file))
                        count += 1
                    except: pass
            for dir in dirs:
                if dir in self._garbage_paths:
                    try:
    ...
    async def watch_system(self: "Sentinel", time: int) -> None: #* Can technically be called by itself, however, it wouldn't be multithreaded.
        ...
        while self.monitoring: # Monitor the system as long as the script is running.
            if self.authorized: # Only monitor the system if the script is available and ready.
